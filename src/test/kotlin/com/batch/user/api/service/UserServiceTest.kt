package com.batch.user.api.service
...
class UserServiceTest {
    ...
    fun test_update_user_ok() {
        ...
        val newUser = UpdateUserRequest(id = oldUser.id!!, imageUrl = "http://qagile.com.br/eletroshop.png")
    ...
    fun test_update_user_error() {
        val newUser = UpdateUserRequest(id = "5e0641f3c8f94e5ddad6591f",
                name = "Rock in Rio Brasil",
                imageUrl = "http://qagile.com.br/eletroshop.png")
        try {
            userService.updateUser(newUser, "q11qq1q1q1q1").toFuture().get()
        } catch (ex: Exception) {
            Assert.assertEquals("com.qagile.quser.api.entities.exception.EventException: O usuário não está cadastrado!", ex.message)
        }
    }
